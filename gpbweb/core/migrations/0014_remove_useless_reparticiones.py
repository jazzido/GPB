...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for rs in orm.ReparticionSinonimo.objects.all():
            try:
                r = orm.Reparticion.objects.get(nombre=rs.nombre)
                print "Moviendo compras de %s a %s" % (r.nombre, rs.canonico.nombre)
                for compra in r.compra_set.all():
                    compra.destino = rs.canonico
                    compra.save()
                print "Borrando %s" % r
                r.delete()
